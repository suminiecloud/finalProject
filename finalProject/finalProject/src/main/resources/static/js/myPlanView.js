...
// 로그인한 회원 아이디
let memid = null;
let loginMidEl = document.getElementById('inputLoginMnickname');
function searchMem() {
	...
	$.ajax({
		...
		success: function(result) {
			...
			for (let res of result) {
				...
				if (loginMidEl.value != res.mnickname && !resultMnickname.includes(res.mnickname)) {
			...
			console.log(resultMnickname);
				resultMemberView(resultMnickname);
			}
...
function resultMemberView(nicknameList) {
	...
	for (let nickname of nicknameList) {
		...
		createDiv.innerHTML =
			`
			<input type="radio" checked class="btn-check checkMemBtn" id="${memid}" name="mid" value="${memid}" autocomplete="off">
            <label class="btn btn-outline-success"  for="${memid}" id="my_btn"  onclick="selectMid('${memid}')">${nickname}</label><br>
            `;
		showMemberDiv.appendChild(createDiv);
	}
	// mid 라디오 버튼 바로 클릭
	if(document.getElementById('my_btn') != null){
