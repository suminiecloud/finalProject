...
function selDate(date) {
	...
	if (vrpList[selDateList] != undefined) {
		...
		if (document.getElementsByClassName("List").length >= 2) {
			saveDiv.innerHTML = `<button type="button" id="save" class="btn btn-dark w-100" onclick="save()">저장하기</button>`;
...
function addMarker(node) {
	...
	if (node.categoryGroupName === '숙박') {
		if (HOTELMARKERS[selDateList]) {
			...
			/*if (HOTELMARKERS[tomorrow_Date]) {
				HOTELMARKERS[tomorrow_Date].setMap(null);
			}*/
			/*HOTELMARKERS[tomorrow_Date] = marker;
			HOTELMARKERS[tomorrow_Date].setMap(map);*/
		}
